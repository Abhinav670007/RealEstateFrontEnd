...
useEffect(()=>{
    const fetchListing = async ()=>{
        ...
        const res = await fetch(`${process.env.REACT_APP_ServerDomain}/listing/getList/${listingId}`)
      ...
      const handleSubmit = async(e)=>{
        e.preventDefault()
          try {
            ...
            const res = await fetch(`${process.env.REACT_APP_ServerDomain}/listing/update/${params.listingId}`,{
                method:"POST",
                headers:{
    ...
    <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
            ...
            <div className="flex flex-wrap gap-6">
                ...
                {formData.offer && (
                    <div className="flex items-center gap-2">
                    <input onChange={handleChange} value={formData.discountPrice} type="number"id='discountPrice' min={0} max={10000} required className='p-3 border border-gray-400 rounded-lg'/>
                    <div className='flex flex-col items-center'>
                        <p>Discount Price</p>
                        <span className='text-sm'>$ / month</span>
                        </div>
        ...
        <div className="flex flex-col flex-1 gap-4">
            ...
            <button className='p-3 bg-slate-600 text-white rounded-lg uppercase'>{loading ? 'Creating....' : 'Update Listing'}</button>
